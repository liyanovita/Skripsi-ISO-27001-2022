<section>
    ...
    <form method="post" action="{{ route('profile.password.update') }}" class="space-y-4">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            {{-- New Password --}}
            <div class="space-y-1.5">
                <label for="password" class="block text-[9px] font-bold text-slate-400 uppercase tracking-widest ml-1">{{ __('New Password') }}</label>
            ...
            {{-- Confirm Password --}}
            <div class="space-y-1.5">
                <label for="password_confirmation" class="block text-[9px] font-bold text-slate-400 uppercase tracking-widest ml-1">{{ __('Confirm New Password') }}</label>
        ...
        <div class="flex items-center gap-4 pt-2">
            <button type="submit" class="px-6 py-2.5 bg-slate-900 text-white rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-blue-600 transition-all shadow-lg active:scale-95">{{ __('Update Password') }}</button>
