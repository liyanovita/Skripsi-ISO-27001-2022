...
@section('title', 'Add ISO Standard')
...
<div class="max-w-3xl">
    ...
    <div class="flex items-center gap-3 mb-6">
        ...
        <div>
            <h2 class="text-lg font-black text-slate-800">Add Standard / Control</h2>
            <p class="text-xs text-slate-400 font-medium">Create a new clause or Annex A control for ISO 27001:2022 structure.</p>
        </div>
    </div>

    <form method="POST" action="{{ route('admin.standards.store') }}" class="space-y-5" x-data="standardForm()">
        @csrf
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <select name="type" required class="form-input w-full px-3.5 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm cursor-pointer">
                        <option value="clause" {{ old('type') == 'clause' ? 'selected' : '' }}>Clause (Klausul 4-10)</option>
                        <option value="control" {{ old('type') == 'control' ? 'selected' : '' }}>Control (Annex A Kontrol)</option>
                    </select>
        ...
        <div class="section-card p-6 space-y-5">
            ...
            <div>
                ...
                <textarea name="implementation_guidance" rows="3"
                    ...
                    class="form-input w-full px-3.5 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm resize-none">{{ old('implementation_guidance') }}</textarea>
        ...
        <div class="flex justify-between items-center pt-2">
            ...
            <button type="submit"
                ...
                <i class="fa-solid fa-floppy-disk text-xs"></i> Save Standard
            </button>
        </div>
...
function standardForm() {
    return {
        questions: @json(old('questions', [''])).map((q, i) => ({ id: Date.now() + i, text: q })),
        addQuestion() {
