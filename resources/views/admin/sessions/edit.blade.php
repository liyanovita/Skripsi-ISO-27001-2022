...
@section('title', 'Edit Audit Session — ' . $session->name)
...
<div class="max-w-4xl space-y-6 pb-12">
    ...
    <div class="flex items-center justify-between">
        ...
        <div class="flex items-center gap-2">
            <span class="text-xs font-medium text-slate-400">Session ID: #{{ $session->id }}</span>
        </div>
    </div>
    ...
    <form method="POST" action="{{ route('admin.sessions.update', $session) }}" class="space-y-6">
        ...
        <div class="bg-white rounded-3xl p-6 sm:p-7 border border-slate-200/80 shadow-sm relative overflow-hidden flex items-center justify-between gap-4">
            <div class="flex items-center gap-4">
                ...
                <div>
                    <h1 class="text-xl font-black text-slate-900 tracking-tight">{{ __('Edit Audit Session') }}</h1>
                    <p class="text-xs text-slate-400 font-medium mt-0.5">{{ __('Update session details, assigned PIC, and status stage') }}</p>
                </div>
            </div>
            <div class="hidden sm:block">
                <span class="inline-flex items-center gap-1.5 px-3.5 py-1.5 rounded-full text-xs font-bold uppercase tracking-wider border
                    ...
                    <span class="w-2 h-2 rounded-full {{ $session->status === 'completed' ? 'bg-emerald-500 animate-pulse' : ($session->status === 'in_progress' ? 'bg-blue-500 animate-pulse' : 'bg-slate-400') }}"></span>
        ...
        <div class="bg-white rounded-3xl border border-slate-200/80 p-6 sm:p-7 shadow-sm space-y-6">
            <div class="flex items-center gap-3 border-b border-slate-100 pb-4">
                ...
                <div>
                    <h2 class="text-base font-bold text-slate-900">{{ __('PIC & Lifecycle Status') }}</h2>
                    <p class="text-xs text-slate-400">{{ __('Assign primary PIC and update session lifecycle stage') }}</p>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                ...
                <div class="space-y-1.5">
                    <label class="block text-xs font-bold text-slate-700 uppercase tracking-wider">
                        {{ __('Audit Status Stage') }} <span class="text-rose-500">*</span>
                    </label>
                    <div class="relative">
                        @php
                            $currentStatus = old('status', $session->trashed() ? 'archive' : $session->status);
                        @endphp
                        <select name="status" required
                            ...
                            <option value="draft" {{ $currentStatus == 'draft' ? 'selected' : '' }}>Draft (Preparatory / Private)</option>
                            <option value="in_progress" {{ $currentStatus == 'in_progress' ? 'selected' : '' }}>Active (Public to Auditee)</option>
                            <option value="completed" {{ $currentStatus == 'completed' ? 'selected' : '' }}>Completed (Finalized / Read-Only)</option>
                            <option value="archive" {{ $currentStatus == 'archive' ? 'selected' : '' }}>Archive (Soft-Deleted / Hidden)</option>
                        </select>
